const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:5530';
...
const adminCookie = await login('admin@mortgage-brokerage-operations.local', 'admin123');
const managerCookie = await login('manager@mortgage-brokerage-operations.local', 'manager123');
const analystCookie = await login('analyst@mortgage-brokerage-operations.local', 'analyst123');
